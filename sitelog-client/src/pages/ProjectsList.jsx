              ...
              <div key={p._id || p.id} className="relative group">
                ...
                {['PM', 'project_manager', 'admin', 'SuperAdmin', 'owner', 'Owner'].includes(user?.role) && (
                  <div className="absolute top-2 right-2 flex opacity-0 group-hover:opacity-100 transition-opacity gap-1 z-10">
                    <button onClick={(e) => { e.preventDefault(); openEdit(p); }} className="p-1.5 bg-navy/5 dark:bg-black/50 hover:bg-orange/20 border border-navy/10 dark:border-white/10 rounded-md text-navy/80 dark:text-white/80 hover:text-orange transition shadow-sm backdrop-blur">
                      <Pencil className="h-4 w-4" />
                    </button>
                    <button onClick={(e) => { e.preventDefault(); openDelete(p); }} className="p-1.5 bg-navy/5 dark:bg-black/50 hover:bg-danger/20 border border-navy/10 dark:border-white/10 rounded-md text-navy/80 dark:text-white/80 hover:text-danger transition shadow-sm backdrop-blur">
